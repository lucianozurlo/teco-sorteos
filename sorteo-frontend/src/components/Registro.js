...
function Registro () {
  ...
  // ================================
  // FILTROS Y ORDENAMIENTO PARA LISTA DE SORTEOS (Realizados)
  // ================================
  ...
  useEffect (
    () => {
      // Opciones para sorteo (resultados)
      const nombresSorteo = Array.from (
        new Set (
          resultados.map (r => r.sorteo && r.sorteo.nombre).filter (Boolean)
        )
      );
        ...
        new Set (
          resultados
            .map (
              r =>
                r.participante
                  ? `${r.participante.nombre} ${r.participante.apellido}`
                  : ''
            )
            .filter (Boolean)
        )
      ...
      const nombresPremio = Array.from (
        new Set (
          resultados.map (r => r.premio && r.premio.nombre).filter (Boolean)
        )
      );
  ...
  // ================================
  // OPCIONES DE FILTRO PARA LISTA DE SORTEOS (Realizados)
  // ================================
  useEffect (
    () => {
