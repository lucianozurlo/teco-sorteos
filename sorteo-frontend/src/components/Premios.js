  ...
  const guardarEdicion = async () => {
    ...
    try {
      const response = await fetch (
        `${API_BASE_URL}/api/premios/${editPremioId}/`,
        {
          method: 'PUT',
          headers: {'Content-Type': 'application/json'},
      ...
      if (response.ok) {
        ...
        setPremios (
          premios.map (p => (p.id === editPremioId ? updatedPremio : p))
        );
        cancelarEdicion ();
        toast.success (
          `Premio "${updatedPremio.nombre}" actualizado exitosamente.`
        );
      } else {
        const errorData = await response.json ();
                    ...
                    <td>
                      {editPremioId === premio.id
                        ? <div className="acciones edit">
                            ...
                            <button onClick={cancelarEdicion} className="rojo">
                              Cancelar
                            </button>
                          </div>
